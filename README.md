...
- `app/api/cron/engage/route.ts` - scheduled endpoint; finds relevant X posts, likes one strong-fit post, posts one AI-generated reply, and optionally follows the author on each run.
...
- The growth cron uses likes + one AI reply per run, then optionally follows selective authors to build visibility more safely.
